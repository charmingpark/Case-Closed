describe('Object에 대해서 학습합니다.', function () {
  ...
  */
  it('Object의 기본을 확인합니다.', function () {
    ...
    expect(Object.keys(emptyObj).length).to.equal(0);
    // STUDY: Object의 길이는 keys의 length로 처리한다.
    ...
    expect(Object.keys(megalomaniac).length).to.equal(5);
  ...
  it("'this'는 method를 호출하는 시점에 결정됩니다.", function () {
    ...
    // expect(currentMonth).to.equal(9);
    // // STUDY: 0월 ~11월 // 5월 일 때, 4 가 나옴.
    
    // expect(currentDate).to.equal(28);
    // // STUDY: 1~ 31
    
    // expect(currentDay).to.equal(4);
    // // STUDY: 0 =일요일 , 6 = 토요일

    expect(currentYear).to.equal(2021);
    expect(megalomaniac.calculateAge(currentYear)).to.equal(2021 - 1970);

    megalomaniac.birthYear = 2000;
    expect(megalomaniac.calculateAge(currentYear)).to.equal(2021 - 2000);

    megalomaniac.changeBirthYear(2010);
    expect(megalomaniac.calculateAge(currentYear)).to.equal(2021 - 2010);
  ...
  it('객체의 method를 정의하는 방법을 확인합니다.', function () {
    ...
    console.log(megalomaniac.battleCry(3))
